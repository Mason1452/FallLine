...
- **calibration follow-ups 执行结果（2026-09-15/16）**：
  ...
  - **【✅ §4.4 第一批标注完成 2026-09-16】10 份未标注 middle 边界样本已人工看片标注**，登记表+分析见 [calibration_anchors.md](file:///Users/mingsen/Project/FallLine/annotations/calibration_anchors.md) 的 "2026-09-16 Boundary Batch"（真实样本池在根目录 video/，`SkiAnaylze/testvideo/` 为空）。分数档位 7/10 一致，3 处偏差全为高估无低估（HI1 92→高质量、M2/M3 72→中级）。两个关键定性结论：**(a) 2D 板身 carvingConfidence 对"是否刻滑"不可信**——教练确认刻滑的 HI1/2/3 其 carvingCnf 仅 28/10/41%，而推坡的 L1 反达 73%，彻底颠倒，佐证 P8-A 退役 sideslip cap、板身不能作硬阈值；**(b) calfLean 或能区分刻滑**（刻滑3份 calf≥50.5，非刻滑7份≤46.5，~48-50 有缝隙，n=10 待扩样验证）。M2/M3 与被认可的 M4 在 edge/calf 上仅差 1.4-4.6，**本批无低风险新评分规则**，继续扩到 ~30 份再动阈值/sigmoid。另发现 HI2 数字91=专业但 stage 文案被 calf≥65 advanced 门槛卡在"高质量"（数字对、标签低半档），列为后续复核点。
