#!/usr/bin/env python3
"""Phase 0 C1 可视化：裸踝轨迹 vs 相机补偿轨迹接触图（等比例）。

输出 outputs/edge_spike/p0_comp_track_sheet.png
用法：python3 scripts/p0_compensated_pathshape_plot.py
"""
...
OUT = ROOT / "outputs/edge_spike/p0_comp_track_sheet.png"
MIN_CNF = 0.30
...
def cum_path(fi, x, y, mot, compensated):
    """累计轨迹；补偿列在 regOK!=1 的对处断开（每段从 0 起）。"""
    segs = [[(0.0, 0.0)]]
...
def main():
    ...
    for r, (alias, rel, grp) in enumerate(CLIPS):
        ...
        for c, comp in enumerate([False, True]):
            ax = axes[r][c]
            segs = cum_path(fi, x, y, mot, comp)
            allx = [p[0] for s in segs for p in s]
            ally = [p[1] for s in segs for p in s]
            for s in segs:
                sx = [p[0] for p in s]; sy = [p[1] for p in s]
                ax.plot(sx, [-v for v in sy], color=color, lw=1.2, alpha=0.9)
            if allx:
            ...
            ax.grid(alpha=0.2)
            if r == 0:
                ax.set_title("raw ankle path", fontsize=10)
                axes[r][1].set_title("camera-compensated", fontsize=10)
        axes[r][0].set_ylabel(f"{alias}\n{grp}", fontsize=8, rotation=0,
