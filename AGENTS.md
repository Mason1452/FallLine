...
- **Report determinism (2026-09-10 P9-A/B)**: `ReportGenerator.dominantPhaseRawValue(from:)` 稳定挑选每个 TurnSegment 的"主要阶段"文案。旧实现 `phaseDistribution.max { ... }` 依赖 Swift Dictionary 无序迭代，同频 tie 时报告文案会跨轮抖动（弯中承压 ↔ 出弯释放）；现按频率降序 + 语义优先级 shaping > initiation > release > transition 二次排序。只影响文案标签，不影响任何评分或 JSON 结构，由 `ReportGeneratorPhaseTieBreakTests` 的 9 条用例守护（含 2000 次稳定性 fuzz）。**P9-B (2026-09-10)** 修复同型 bug：`CenterOfMassFitCalculator.dominantIssue(from:)` 顶层重心主问题标签的 `counts.max { ... }` 在"过低 vs 偏高"同频 tie 时也依赖 hash-seed 抽奖，抖动面比 P9-A 更大（影响整个视频的顶层 `mainIssue`）。现按频率降序 + 语义优先级 **偏高 > 过低** 二次排序（依据：偏高对 `score` 的惩罚斜率显著更陡，且教练视角判定优先级更高）。由 `CenterOfMassFitCalculatorTieBreakTests` 的 9 条用例守护（含 2000 次稳定性 fuzz）。**通用规约**：任何依赖 `Dictionary`/`Set` 归约影响用户可见输出（报告 / JSON / 分数）的位置，都必须以显式排序键收尾，且当主键 tie 时必须提供业务语义优先级作为二次排序键。
