...
GOOD_A 从 94.1 → 86 是本次唯一的**下迁**样本，需要判断是"合理重新校准"还是"过度惩罚"：
- 2026-05 时点：evidence cap 更宽松 + 无 sigmoid，calf 48 的原始分接近 60（线性）
- 2026-09-15 时点：sigmoid c=40 让 calf=48 只得 55 分（对应 40° 中点略高），权重 0.35 拉低整体

**判断**：GOOD_A 是"低姿态刻画"（弯中承压阶段主导，姿态干净但立刃角度不深），教练锚点 94 分应保留。**若采纳 §4.1 + §4.2**（不动 sigmoid），GOOD_A 走 calf=48 不落双维兜底、knee=98 不触发加权，分数应保持 86 附近，仍是-8。**建议在方案 4.1/4.2 落地后重跑一次锚点验证再决定是否调 sigmoid**（当前 c=40 是保护主 corpus v5 不跃档的必要值）。
