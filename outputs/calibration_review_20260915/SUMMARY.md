...
### 4.4 【长期 · 已给出可执行方案 2026-09-16】扩样本 + 建立分档基准

**为什么必须扩样本**：§4.2 证明当前 18 份里"压 BAD_ACC 与保 v6"仅靠 0.44-0.9 分的测量缝隙；§4.3 高边界 GOOD_A 与 MID_ACC6 在 avg/calf 上仅差 2.1/1.7。这些都不是规则问题，而是**边界样本密度不足**——单点阈值必然过拟合，需要统计分布才能定阈值/重训 sigmoid。

**样本池盘点（更正）**：真实 corpus 在根目录 [video/](../../video/)（`SkiAnaylze/testvideo/` 当前为空，原 follow-up 引用有误）：

| 桶 | 总数 | 已作锚点 | 未标注候选 |
|---|---:|---:|---:|
| video/middle | 20 | 10 | **10** |
| video/good | 16 | 1 | 15 |
| video/bad | 9 | 1 | 8 |

**第一批候选（10 份未标注 middle，已用当前 release 跑出预测分，按分排序）**：

| 文件（video/middle/） | 当前预测分 | 当前阶段 | 标注价值 |
|---|---:|---|---|
| `9714be3aba73f5f94130750c2a15d381.MP4` | **92** | 高质量 | ⭐ 最高优先：middle 桶却 92，判定是真专业还是 MID_ACC7 式虚高 |
| `4f9ec73b994b63b0775ccfb7a8ef7e6f.MP4` | **91** | 高质量 | ⭐ 同上（旧版报告仅 60，跨版本漂移 +31） |
| `b343b317a6689df373085ec85e042480.MP4` | **87** | 高质量 | ⭐ 专业档下限，校准 GOOD_A 86 是否合理 |
| `v0200fg10000d318sovog65g16lolm8g.MP4` | 78 | 稳定滑行 | 高：中级顶边界（≈78 锚点） |
| `v0200fg10000coc7bcrc77u875h9q7sg.MP4` | 72 | 稳定滑行 | 中：中级中 |
| `v0300fg10000d6fe0nnog65jmdun29vg.MP4` | 72 | 稳定滑行 | 中：中级中 |
| `v0d00fg10000cu3omkvog65r22seg0t0.MP4` | 72 | 稳定滑行 | 中：中级中 |
| `v1e00fgi0000d5bksdfog65irrhr4bog.MP4` | 65 | 稳定滑行 | 低：中级下 |
| `v0300fg10000d5h313fog65nermuqklg.MP4` | 62 | 稳定滑行 | 低：中级底（≈58-62 锚点） |
| `v2800fgi0000d54jhefog65lbfdhma2g.MP4` | 62 | 稳定滑行 | 低：中级底 |

**教练标注协议（每份需人工给出）**：
1. **五档档位**：初级 <60 / 中级 [60,70) / 中级偏上 [70,80) / 高质量 [80,88) / 专业 ≥88；
2. **是否稳定刻滑**（布尔）——直接决定 §4.2 edge/pressure cap 是否该压；
3. **三个 90+ 样本专项问题**："立刃深度、弯形、姿态干净度"三项各打分，用于判定它们是真专业还是评分虚高（这是解开 GOOD_A vs MID_ACC7 死结的关键）；
4. 标注"左转 vs 右转"是否均衡，为 §4.5 跨弯对称特征储备真值。

**落地后解锁**：
- 用 ≥30 份带档位样本拟合 sigmoid c（当前 c=40 是主 corpus 拍脑袋值）；
- 用 edge/pressure 在"真刻滑 vs 非刻滑"两类上的分布，重新求 §4.2 的 cap 阈值（从统计 margin 而非单点 0.44 缝隙出发）；
- 用高边界样本分布决定 §4.3 `avg≥80` 是否可安全分离；
- 训练/验证跨弯对称特征（§4.5）。

**建议标注顺序**：先 3 份 90+（解决最尖锐的虚高争议，ROI 最高）→ 再 78 边界 1 份 → 其余 6 份补密度。人工成本约 10 × 2-3 分钟看片。
