...
final class PoseScorerTests: XCTestCase {
    ...
    /// Tick 3 (2026-09-15) sigmoid 后 calfLean(75°) 从线性 93.75 变为 98.20：
    ///   ≈ 61.2*0.25 + 100*0.15 + 98.20*0.35 + 92.5*0.15 + 100*0.10 = 88.545
    /// Tick 2 权重重分配后期望值从 87.55 → Tick 3 sigmoid 后 ≈ 88.55。
    func test_p5b_kneeBend61_noCap() {
        ...
        // kneeBend > 60 时无 cap，totalScore 应 = rawScore ≈ 88.55（未被截断到 88 或以下）
        XCTAssertEqual(result.totalScore, 88.55, accuracy: 0.5,
                       "kneeBend>60 无 cap，totalScore 应 = rawScore ≈ 88.55，实测 \(result.totalScore)")
    }
